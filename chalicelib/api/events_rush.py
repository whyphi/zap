...
def register_routes(events_rush_service: EventsRushService):
    ...
    @events_rush_api.route(
        "/events/rush/checkin/{event_id}", methods=["POST"], cors=True
    )
    @handle_exceptions
    ...
    @events_rush_api.route(
        "/events/rush/timeframe/default", methods=["POST"], cors=True
    )
    @handle_exceptions
    def get_rush_events_default_timeframe():
        data = events_rush_api.current_request.json_body
        rushee_id = data["rushee_id"]
        return events_rush_service.get_rush_events_default_timeframe(
            rushee_id=rushee_id
        )
    ...
    def get_rush_timeframe_analytics(timeframe_id):
        return events_rush_service.get_rush_timeframe_analytics(
