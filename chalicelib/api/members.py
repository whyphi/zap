...
def register_routes(member_service: MemberService):

    @members_api.route("/members/{user_id}", methods=["GET"], cors=True)
    ...
    @members_api.route("/members/{user_id}", methods=["PUT"], cors=True)
    @auth(members_api, roles=[Roles.MEMBER, Roles.ADMIN])
    @handle_exceptions
    def update_member(user_id):
        data = members_api.current_request.json_body
        headers = members_api.current_request.headers
        return member_service.update(user_id=user_id, data=data, headers=headers)
    ...
    def onboard_member(user_id):
        ...
        # TODO: If is_new_user is False, reject onboarding
